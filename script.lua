...
local SaveList = {
    game:GetService('Workspace'), game:GetService('ReplicatedFirst'),
    game:GetService('ReplicatedStorage'), game:GetService('Lighting'),
    game:GetService('StarterGui'), game:GetService('StarterPack'),
    game:GetService('StarterPlayer'), game:GetService('Teams') -- game:GetService('InsertService');
};

local IgnoredList = {
    'CameraScript', 'ControlScript', 'ChatScript', 'BubbleChat', 'Camera'
}
...
for _, Player in next, Players:GetPlayers() do
    if (Player ~= Me) then table.insert(IgnoredList, tostring(Player)); end
end
...
function InNil() return getnilinstances(); end
function WritesFl(Location, Data) return writefile(Location, Data); end
function Decompile(Script)
    return "cannot decompile scripts - depso" -- decompile(Script);
end
warn = warn;
  ...
  if (Tag == 'deprecated') or (Tag == 'readonly') then
  	...
  	break;
        ...
        if (not Ignore) then
            ...
            if Ignored and Ignored[Val.Name] then Ignore = true; end
...
local function PropsOf(Obj)
    if Props[Obj.ClassName] then return Props[Obj.ClassName]; end
    ...
    table.sort(Prop, function(A, B) return A.Name < B.Name; end);
...
local function SetParent(Obj, Parent)
    ...
    if Obj.Archivable then Cloned = Obj:Clone(); end
...
local function SavePlaceAsync()
    ...
    local function SaveInstance(Obj)
        if Classes[Obj.ClassName] and (not IgnoredList[Obj.Name]) then
            ...
            Final[#Final + 1] = '<Item class="' .. Obj.ClassName ..
                                    '" referent="RBX' .. Num .. '"><Properties>';

            if ((Num % 1080) == 0) then Beat:wait(); end
            ...
            for _, Prop in next, Props do
                ...
                elseif (typeof(Objp) == 'EnumItem') then
                    Append = '<token name="' .. ObjProp .. '">' .. Objp.Value ..
                                 '</token>';
                else
                    if (Type == 'bool') then
                        Append = '<bool name="' .. ObjProp .. '">' ..
                                     tostring(Objp) .. '</bool>';
                    elseif (Type == 'float') then
                        Append = '<float name="' .. ObjProp .. '">' ..
                                     tostring(Objp) .. '</float>';
                    elseif (Type == 'int') then
                        Append = '<int name="' .. ObjProp .. '">' ..
                                     tostring(Objp) .. '</int>';
                    ...
                    elseif (Type == 'BrickColor') then
                        Append =
                            '<int name="' .. ObjProp .. '">' .. Objp.Number ..
                                '</int>';
                    elseif (Type == 'Vector2') then
                        Append =
                            '<Vector2 name="' .. ObjProp .. '">' .. '<X>' ..
                                Objp.x .. '</X>' .. '<Y>' .. Objp.y .. '</Y>' ..
                                '</Vector2>'
                    elseif (Type == 'Vector3') then
                        Append =
                            '<Vector3 name="' .. ObjProp .. '">' .. '<X>' ..
                                Objp.x .. '</X>' .. '<Y>' .. Objp.y .. '</Y>' ..
                                '<Z>' .. Objp.z .. '</Z>' .. '</Vector3>'
   ...
   .. '</Vector3>']]
    ...
    <Vector3 name="InitialSize">
    	...
    	<Z>999</Z>
                    ...
                    elseif (Type == 'CoordinateFrame') then
                        local X, Y, Z, R00, R01, R02, R10, R11, R12, R20, R21,
                              R22 = Objp:components()

                        Append = '<CoordinateFrame name="' .. ObjProp .. '">' ..
                                     '<X>' .. X .. '</X>' .. '<Y>' .. Y ..
                                     '</Y>' .. '<Z>' .. Z .. '</Z>' .. '<R00>' ..
                                     R00 .. '</R00>' .. '<R01>' .. R01 ..
                                     '</R01>' .. '<R02>' .. R02 .. '</R02>' ..
                                     ...
                                     '</R12>' .. '<R20>' .. R20 .. '</R20>' ..
                                     '<R21>' .. R21 .. '</R21>' .. '<R22>' ..
                                     R22 .. '</R22>' .. '</CoordinateFrame>'
                    elseif (Type == 'Content') then
                        local String = Objp:gsub("[&<>]", Conversions);

                        Append = '<Content name="' .. ObjProp .. '"><url>' ..
                                     String .. '</url></Content>';
                    elseif (Type == 'UDim2') then
                        local Objp = Objp;

                        Append = '<UDim2 name="' .. ObjProp .. '">' .. '<XS>' ..
                                     Objp.X.Scale .. '</XS>' .. '<XO>' ..
                                     Objp.X.Offset .. '</XO>' .. '<YS>' ..
                                     Objp.Y.Scale .. '</YS>' .. '<YO>' ..
                                     Objp.Y.Offset .. '</YO>' .. '</UDim2>'
                    elseif (Type == 'Color3') then
                        Append = '<Color3 name="' .. ObjProp .. '">' .. '<R>' ..
                                     Objp.r .. '</R>' .. '<G>' .. Objp.g ..
                                     '</G>' .. '<B>' .. Objp.b .. '</B>' ..
                                     '</Color3>'
                    elseif (Type == 'NumberRange') then
                        Append = '<NumberRange name="' .. ObjProp .. '">' ..
                                     tostring(Objp.Min) .. ' ' ..
                    ...
                    elseif (Type == 'NumberSequence') then
                        ...
                        for i, v in next, Objp.Keypoints do
                            Ob[#Ob + 1] =
                                tostring(v.Time) .. ' ' .. tostring(v.Value) ..
                                    ' ' .. tostring(v.Envelope) .. ' ';
                        end
                    ...
                    elseif (Type == 'ColorSequence') then
                        ...
                        for i, v in next, Objp.Keypoints do
                            Ob[#Ob + 1] = table.concat {
                                tostring(v.Time) .. ' ' .. tostring(v.Value.r) ..
                                    ' ' .. tostring(v.Value.g) .. ' ' ..
                                    tostring(v.Value.b), " 0 "
                            };
                        end
                    ...
                    elseif (Type == 'Rect2D') then
                        Append =
                            '<Rect2D name="' .. ObjProp .. '">' .. '<min>' ..
                                '<X>' .. tostring(Objp.Min.X) .. '</X>' .. '<Y>' ..
                                tostring(Objp.Min.Y) .. '</Y>' .. '</min>' ..
                                '<max>' .. '<X>' .. tostring(Objp.Max.X) ..
                                '</X>' .. '<Y>' .. tostring(Objp.Max.Y) ..
                                '</Y>' .. '</max>' .. '</Rect2D>'
                    elseif (Type == 'ProtectedString') then
                        ...
                        Append = '<ProtectedString name="' .. ObjProp ..
                                     '"><![CDATA[' .. Src ..
                                     ']]></ProtectedString>';
                    elseif (Type == 'Object') then
                        ...
                        Append = '<Ref name="' .. ObjProp .. '">' .. Objp ..
                                     '</Ref>';
                    elseif (Type == 'PhysicalProperties') then
                        if Objp then
                            Append = '<PhysicalProperties name="' .. ObjProp ..
                                         '"><CustomPhysics>true</CustomPhysics>' ..
                                         '<Density>' .. tostring(Objp.Density) ..
                                         '</Density>' .. '<Friction>' ..
                                         tostring(Objp.Friction) ..
                                         '</Friction>' .. '<Elasticity>' ..
                                         tostring(Objp.Elasticity) ..
                                         '</Elasticity>' .. '<FrictionWeight>' ..
                                         tostring(Objp.FrictionWeight) ..
                                         '</FrictionWeight>' ..
                                         '<ElasticityWeight>' ..
                                         tostring(Objp.ElasticityWeight) ..
                                         '</ElasticityWeight>' ..
                                         '</PhysicalProperties>'
                        else
                            Append = '<PhysicalProperties name="' .. ObjProp ..
                ...
                if Append then Final[#Final + 1] = Append; end
            end
    ...
    for _, Child in next, SaveList do SaveInstance(Child); end

    Final[#Final + 1] = '</roblox>';

    local Place = game:GetService('MarketplaceService'):GetProductInfo(
                      game.PlaceId);
    ...
    warn(string.format(
             'Saving took %d second(s), please check your workspace folder',
             tick() - Timer));
end
